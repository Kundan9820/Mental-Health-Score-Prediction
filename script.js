...
(() => {
  ...
  // =========================================================
  const API_BASE =
    "https://mental-health-score-prediction-4-g890.onrender.com";
  ...
  // FIX:
  // Your HTML currently has error-copy but may not have error-label.
  // We safely create/use it so JS never crashes on an API error.
  let errorLabelEl = document.getElementById("error-label");
  const errorCopyEl = document.getElementById("error-copy");

  if (!errorLabelEl && errorCopyEl) {
    errorLabelEl = document.createElement("p");
    errorLabelEl.id = "error-label";
    errorLabelEl.className = "error-label";

    errorCopyEl.parentNode.insertBefore(errorLabelEl, errorCopyEl);
  }
      ...
      async (e) => {
        ...
        try {
          ...
          if (
            ...
            message =
              "The backend took too long to respond. Render may be waking the server from sleep. Please try again.";
          } else if (
            err &&
            err.message
          ) {
